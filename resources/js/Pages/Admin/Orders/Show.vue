...
<template>
    <AdminLayout>
        <div class="max-w-4xl">
            ...
            <!-- Status transition -->
            <div v-if="allowedTransitions.length" class="bg-white rounded-2xl shadow-md p-6 mb-6">
                <h2 class="font-bold mb-3">Cập nhật trạng thái</h2>
                <div class="flex items-center space-x-4">
                    <select v-model="selectedStatus" class="rounded-lg border-gray-300 flex-1">
                        <option value="">Chọn trạng thái</option>
                        <option v-for="t in allowedTransitions" :key="t.value" :value="t.value">{{ t.label }}</option>
                    </select>
                    <button @click="updateStatus" :disabled="!selectedStatus" class="px-6 py-2 bg-amber-700 text-white rounded-lg disabled:opacity-50">Cập nhật</button>
                </div>
                <textarea v-if="selectedStatus === 'cancelled'" v-model="cancelReason" rows="2" placeholder="Lý do hủy..." class="w-full rounded-lg border-gray-300 mt-3"></textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="bg-white rounded-2xl shadow-md p-6">
                    ...
                    <p class="text-amber-600 mt-1">{{ order.order_type }}</p>
                </div>
                <div class="bg-white rounded-2xl shadow-md p-6">
            ...
            <div class="bg-white rounded-2xl shadow-md p-6">
                ...
                <div class="space-y-3">
                    <div v-for="item in order.items" :key="item.product_name" class="flex justify-between p-3 bg-gray-50 rounded-xl">
                        <div>
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <hr />
                    <div class="flex justify-between text-lg font-bold"><span>Tổng</span><span class="text-amber-700">{{ formatCurrency(order.total) }}</span></div>
                </div>
