...
<template>
    <AdminLayout>
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-[#1a1a1a] font-serif">Kích thước</h1>
        ...
        <div class="bg-white rounded border border-[#E8D9C5] overflow-hidden shadow-sm">
            <table class="w-full text-sm">
                <thead class="bg-[#FAF6F0]"><tr><th class="px-6 py-3 text-left">Tên</th><th class="px-6 py-3 text-left">Nhãn</th><th class="px-6 py-3 text-center">Thứ tự</th><th class="px-6 py-3 text-right">Thao tác</th></tr></thead>
                <tbody class="divide-y">
                    <tr v-for="s in sizes" :key="s.id" class="hover:bg-[#FAF6F0] transition">
                        <td class="px-6 py-4 font-bold">{{ s.name }}</td>
                        <td class="px-6 py-4">{{ s.label }}</td>
                        <td class="px-6 py-4 text-center">{{ s.sort_order }}</td>
                        <td class="px-6 py-4 text-right space-x-2"><button @click="openEdit(s)" class="text-[#D4A853]">Sửa</button><button @click="deleteSize(s.id)" class="text-red-500">Xóa</button></td>
                    </tr>
                </tbody>
            </table>
        </div>
