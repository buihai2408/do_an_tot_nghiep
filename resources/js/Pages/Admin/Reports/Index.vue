...
<template>
    <AdminLayout>
        <h1 class="text-2xl font-bold text-[#1a1a1a] font-serif mb-6">Báo cáo</h1>
            ...
            <div class="bg-white rounded border border-[#E8D9C5] p-6 shadow-sm">
                ...
                <div class="space-y-3">
                    <div v-for="(count, status) in ordersByStatus" :key="status" class="flex justify-between items-center">
                        ...
                        <span class="font-bold bg-amber-100 text-[#D4A853] px-3 py-1 rounded-full">{{ count }}</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="space-y-3">
                    <div v-for="(p, i) in topProducts" :key="i" class="flex justify-between items-center">
                        <div class="flex items-center space-x-3">
                            <span class="w-6 h-6 bg-amber-100 text-[#D4A853] rounded-full flex items-center justify-center text-xs font-bold">{{ i + 1 }}</span>
            ...
            <div class="bg-white rounded border border-[#E8D9C5] p-6 shadow-sm">
                ...
                <div class="space-y-3">
                    <div v-for="(c, i) in topCustomers" :key="i" class="flex justify-between items-center">
                        ...
                        <span class="font-medium text-[#D4A853]">{{ formatCurrency(c.total_spent) }}</span>
                    </div>
                </div>
            </div>
        </div>
